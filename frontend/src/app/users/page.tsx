...
export default function UsersPage() {
  ...
  const getRoleColor = (role: string) => role === 'admin' ? 'bg-purple-50 text-purple-700' : 'bg-[var(--primary-light)] text-[var(--primary)]';
  const getInitials = (name: string) => name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
  const getAvatarColor = (name: string) => {
    const c = ['from-[#B85C38] to-[#9C4D2F]','from-purple-500 to-purple-600','from-emerald-500 to-emerald-600','from-orange-500 to-orange-600','from-pink-500 to-pink-600'];
      ...
      <div className="space-y-4 lg:space-y-6 max-w-4xl mx-auto h-full overflow-y-auto pb-6">
        ...
        <div className={`flex items-center justify-between gap-3 transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
          <div>
            ...
            <h1 className="text-xl lg:text-2xl font-semibold text-[var(--text)] tracking-tight">Usuários</h1>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="flex items-center gap-2 px-3 lg:px-4 py-2.5 bg-[var(--primary)] text-white text-[13px] font-medium rounded-xl hover:bg-[var(--primary-hover)] hover:shadow-lg hover:shadow-[var(--primary)]/20 active:scale-[0.98] transition-all"
          >
        ...
        <div className={`grid grid-cols-3 gap-3 lg:gap-4 transition-all duration-700 ease-out delay-75 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <div className="bg-white rounded-2xl p-3 lg:p-4 border border-gray-100 flex items-center gap-3">
            <div className="w-9 h-9 lg:w-10 lg:h-10 bg-[var(--primary-light)] rounded-xl flex items-center justify-center flex-shrink-0">
              <Users className="w-4 h-4 lg:w-[18px] lg:h-[18px] text-[var(--primary)]" />
            </div>
            <div>
              <p className="text-lg lg:text-xl font-bold text-[var(--text)] tabular-nums">{users.length}</p>
          ...
          <div className="bg-white rounded-2xl p-3 lg:p-4 border border-gray-100 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg lg:text-xl font-bold text-[var(--text)] tabular-nums">{activeCount}</p>
          ...
          <div className="bg-white rounded-2xl p-3 lg:p-4 border border-gray-100 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg lg:text-xl font-bold text-[var(--text)] tabular-nums">{adminCount}</p>
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <p className={`font-medium text-[13px] truncate ${u.is_active ? 'text-[var(--text)]' : 'text-gray-400'}`}>
                        {u.name}
                        {u.id === user?.id && (
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
            <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl mx-4 border border-gray-100" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-lg font-semibold text-[var(--text)]">Novo Usuário</h2>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder:text-gray-400 focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary-light)] focus:bg-white outline-none transition-all"
                    />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-12 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder:text-gray-400 focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary-light)] focus:bg-white outline-none transition-all"
                    />
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className={`flex-1 py-2.5 rounded-xl text-[13px] font-medium border transition-all ${
                        newRole === 'atendente' ? 'border-[var(--primary)] bg-[var(--primary-light)] text-[var(--primary)]' : 'border-gray-200 text-gray-400 hover:bg-gray-50'
                      }`}
                    >
                      Atendente
                    </button>
              ...
              <button
                ...
                className="w-full mt-6 py-3 bg-[var(--primary)] text-white font-medium rounded-xl hover:bg-[var(--primary-hover)] hover:shadow-lg hover:shadow-[var(--primary)]/20 active:scale-[0.98] transition-all disabled:opacity-40 disabled:active:scale-100 flex items-center justify-center gap-2"
              >
